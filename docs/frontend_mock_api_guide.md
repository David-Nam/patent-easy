...
현재 백엔드는 프론트엔드 개발을 막지 않기 위해 Mock API를 제공합니다. 프론트엔드는 실제 KIPRIS/OpenAI 연동 여부와 관계없이 아래 API 계약으로 검색, 상세, 요약 화면을 먼저 개발할 수 있습니다.
...
KIPRIS key는 `scripts/verify_kipris_api.py`를 실행하거나 실제 KIPRIS client를 API에 연결할 때만 필요합니다. OpenAI key는 실제 키워드 추출, 요약, 재정렬을 OpenAI API로 교체할 때만 필요합니다.
