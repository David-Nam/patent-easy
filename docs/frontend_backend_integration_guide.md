...
현재 서버는 다음 상태로 배포되어 있습니다.

| 기능 | 현재 동작 |
|---|---|
| 검색 | 실제 Gemini Query Builder와 KIPRIS 검색 API를 호출합니다. |
| 상세 조회 | `data/mock_patents.json`의 local mock 상세 데이터를 반환합니다. |
| 요약 | 실제 KIPRIS 서지/청구항 조회 후 Gemini로 요약합니다. |

중요한 제한사항이 있습니다. 검색 결과는 실제 KIPRIS 결과이지만,
`GET /api/v1/patents/{patent_id}`는 아직 실제 KIPRIS 상세 API와 연결되지 않았습니다.
따라서 검색 결과의 `patent_id`로 상세 조회를 호출하면 `404 PATENT_NOT_FOUND`가
나올 수 있습니다. 프론트엔드는 이 경우 “상세 정보는 준비 중입니다” 같은 안내를
보여주면 됩니다.
...
| `GET` | `/api/v1/patents/{patent_id}` | 특허 상세 조회 | 상세 화면, 현재 mock data |
...
```bash
curl -s https://patent-easy-api.onrender.com/api/v1/patents/10-2023-0098765
```
...
- `relevance_score`는 0부터 100까지의 정수입니다.

상세 화면:

- 현재 상세 조회는 mock data 기반입니다.
- 검색 결과 ID로 상세 조회가 실패할 수 있으므로 `PATENT_NOT_FOUND`를 정상적인 준비 중 상태로 처리합니다.
...
검색, 상세, 요약 함수를 만들고 기존 화면의 loading/error/empty 상태까지 연결해줘.
KIPRIS_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY는 프론트엔드에 추가하지 마.
상세 조회는 현재 mock data이므로 PATENT_NOT_FOUND를 준비 중 상태로 처리해줘.
```
...
- 상세 조회 `404 PATENT_NOT_FOUND`를 깨진 화면이 아니라 안내 상태로 처리한다.
