...
| Query Builder live | `RUN_LIVE_LLM=1 venv/bin/python -m pytest tests/test_query_builder_live.py -m live_llm -s` | Gemini | 자연어 검색어 추출용 Gemini structured output 검증 |
...
| 챗봇 live | `RUN_LIVE_KIPRIS=1 RUN_LIVE_LLM=1 venv/bin/python -m pytest tests/test_chat_live.py -m "live_kipris and live_llm" -s` | KIPRIS + LLM | KIPRIS 상세/청구항 조회와 단일 특허 Q&A 통합 검증 |
| 검색 품질 benchmark | `venv/bin/python scripts/benchmark.py --mode mock --cache off` | 없음 | 평가 쿼리 기준 Precision@10, recall, latency 산출 |
| Render 배포 smoke test | `DEPLOYED_API_BASE_URL=https://patent-easy-api.onrender.com venv/bin/python scripts/smoke_test_deployed_api.py` | Render + KIPRIS + Gemini | 공개 배포 URL 기준 health, readiness, OpenAPI, 검색, 상세, 요약 확인 |
...
| `tests/test_chat_service.py` | 챗봇 service cache 흐름 검증 |
| `tests/test_chat_api.py` | 챗봇 API 오류 매핑 검증 |
| `tests/test_openapi_contract.py` | OpenAPI 경로, 응답 모델, 에러 status 계약 검증 |
| `tests/test_benchmark.py` | 평가 데이터셋과 benchmark 산출물 검증 |
| `tests/test_deployment_smoke.py` | 배포 smoke test script의 검증 로직 단위 테스트 |
| `tests/test_search_live.py` | 실제 KIPRIS 검색 live 검증 |
| `tests/test_query_builder_live.py` | 실제 Gemini Query Builder structured output 검증 |
| `tests/test_llm_client_live.py` | 실제 Gemini LLM live 검증 |
| `tests/test_summary_live.py` | 실제 KIPRIS + Gemini 요약 live 검증 |
| `tests/test_chat_live.py` | 실제 KIPRIS + Gemini 챗봇 live 검증 |
...
- 챗봇 prompt 또는 `/chat` 계약 변경 후에는 챗봇 live 테스트가 1회 이상 통과합니다.
- Render 배포 후에는 `scripts/smoke_test_deployed_api.py`가 공개 URL 기준으로 통과해야 합니다.
...
- 작업 16에서 평가용 쿼리와 benchmark script를 추가해 정량 품질 지표를 산출합니다.
